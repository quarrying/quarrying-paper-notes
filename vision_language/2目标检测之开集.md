...
## [2024] YOLO-World
---
**Image-Pooling Attention** (I-Pooling Attention) 的作用是利用图像多尺度特征来增强文本特征. 结合文献和代码确定其具体操作是:
- 通过 **线性投影层** 实现各尺度图像特征通道维度的归一化.
- 通过 **自适应池化层** 实现各尺度图像特征空间维度的归一化.
- 在通道维度上串联各尺度图像特征, 并进行适当 reshape, 变成 $\mathrm{batch\_size} \times \mathrm{sequence\_len} \times \mathrm{embedding\_size}$ 的形式.
- 文本特征作为 query, 上述变换后的特征作为 key, value, 进行交叉注意力计算.
- 原始文本特征和上述用图像特征增强后的文本特征进行加权和, 作为最终的文本特征.
