...
### 损失函数
$$\ell_{i} = -\log \frac{\exp(\mathrm{sim}(v_{y_i}, f_i)/\tau)}{\sum_{j=1}^{n} \exp(\mathrm{sim}(v_j, f_i)/\tau)}$$

其中 $n$ 等于数据集中图像的数目, $\mathrm{sim}(\cdot,\cdot)$ 是余弦相似度函数, $\tau$ 是温度参数. $v_i$ 是 memory bank 中的特征, $f_i$ 是网络的输出特征.
...
其中 $f(\cdot)$ 表示 base encoder network, $g(\cdot)$ 表示 projection head.
...
$$\mathcal{L} = - \mathrm{sim}(q_\theta(z_\theta), \mathrm{stopgrad}(z'_\xi)) - \mathrm{sim}(q_\xi(z'_\xi), \mathrm{stopgrad}(z_\theta))$$
...
$$\mathcal{L} = -\frac{1}{2}\mathrm{sim}(p_1, \mathrm{stopgrad}(z_2) - \frac{1}{2}\mathrm{sim}(p_2, \mathrm{stopgrad}(z_1))$$
