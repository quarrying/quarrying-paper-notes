...
但通常情况下, world_size 指的是所有节点的 gpu 的总数, rank 是 gpu 的 rank. 这两个概念具有误导性.

通读代码后, 发现 `main_simsiam.py` 后面对 argument 中的 world_size 和 rank 进行了调整, 调整后的意义与通常情况下的意义一致, 相关代码如下, 困惑解除.
