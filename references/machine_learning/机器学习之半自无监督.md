...
$$\ell_{i,j} = -\log \frac{\exp(\mathrm{sim}(\bm z_i, \bm z_j)/\tau)}{\sum_{k=1,k \neq i}^{2N} \exp(\mathrm{sim}(\bm z_i, \bm z_k)/\tau)}$$
其中 $N$ 是 batch size, $z_i$ 和 $z_j$ 是同一个样本不同数据增强输入的特征.
...
## leveraging temporal continuity via tracking
---

## predicting future
---

## preserving the equivariance of egomotion
---
